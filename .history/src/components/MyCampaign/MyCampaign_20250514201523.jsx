...
const handleDelete = async (id) => {
  ...
  try {
    ...
    const text = await res.text(); // Get response as text (not JSON)
    console.log("Raw response:", text); // Check the raw response from the server

    if (res.ok) {
      try {
        const result = JSON.parse(text); // Try parsing the response as JSON
                ...
                <tr key={campaign._id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <td className="p-3 border dark:border-gray-600">
                    <button
                      ...
                      onClick={() => navigate(`/UpdateCampaign/${campaign.id}`)}
                    >
                      Update
                    </button>
                    <button
