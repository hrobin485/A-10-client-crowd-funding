...
const Navbar = () => {
  ...
  // extra sub‑routes shown only in mobile dropdown
  const dashSub = (
    <ul className="pl-4 flex flex-col gap-2 text-sm">
      <li><NavLink to="/Dashboard/Overview" className={item}>Overview</NavLink></li>
      <li><NavLink to="/Dashboard/MyCampaigns" className={item}>My Campaigns</NavLink></li>
      <li><NavLink to="/Dashboard/Donations" className={item}>Donations</NavLink></li>
      {user?.email === "admin@example.com" && (
        <li><NavLink to="/Dashboard/Admin" className={item}>Admin Panel</NavLink></li>
      )}
    </ul>
  );
  ...
  const mobileLinks = (
    <>
      ...
      <li><NavLink to="/AllCampaign" className={item}>All Campaign</NavLink></li>
      <li>
        <button className="flex items-center justify-between w-full" onClick={() => setDashOpen(!dashOpen)}>
          <span className={normal.replace("hover:", "") + " " + (dashOpen && "font-semibold text-blue-500 dark:text-emerald-400")}>Dashboard</span>
