...
const Login = () => {
  ...
  const [user, setUser] = useState(null); // To store the logged-in user info

  // Monitor user authentication state
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((authUser) => {
      if (authUser) {
        setUser(authUser); // Set the user object if logged in
      } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded shadow-md w-full max-w-md">
        ...
        {/* Conditional rendering: If user is logged in, show profile picture, else show login form */}
        {!user ? (
